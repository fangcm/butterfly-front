...
MobileArea.prototype = {
  ...
  bindEvent: function() {
    ...
    function popupArea(e) {
      _self.gearArea = document.querySelector('.gearArea'+ _self.uuid);
    ...
    function rollGear(target) {
      ...
      target['int_' + target.id] = setInterval(function() {
        ...
        var minTop = -(target.dataset.len - 1) * 4
        if (pos < minTop) {
          pos = minTop
          setDuration()
        }
        if (stopGear) {
          var gearVal = Math.abs(pos) / 4
  ...
  setGearTooth: function(data) {
    ...
    if (l > 0) {
      ...
      for (let i = 0; i < l; i++) {
        ...
        itemStr += "' mark='" + item[i][this.keys['mark']];
        itemStr += "'>" + item[i][this.keys['name']] + '</div>'
      }
      gearChild[_self.index].innerHTML = itemStr
      gearChild[_self.index].style['-webkit-transform'] = 'translate3d(0,' + (-gearVal * 4) + 'em,0)'
      gearChild[_self.index].setAttribute('top', -gearVal * 4 + 'em')
  ...
  finish: function(e) {
    ...
    var provinceMark = areaProvince.childNodes[provinceVal].getAttribute('mark');
    ...
    var cityMark = areaCity.childNodes[cityVal].getAttribute('mark');
    ...
    var countyMark = areaCounty.childNodes[countyVal].getAttribute('mark');
  ...
  setLocation(location) {
    const val = location[this.keys['id']];
    const arr = val.toString().split(' ');
    if (arr.length !== this.column) {
      throw new Error('传入的column值与设定的默认location值格式不匹配')
    }
    const provArr = this.data.filter((one) => {
      return one[this.keys['id']] === Number(arr[0]);
    });

    if (!!!provArr.length) {
      console.error('数据配置信息不对称，当前默认location值不在记录集内');
      return false;
    }
    ...
    if (this.column === 1) {
      ...
      const cityArr = provArr[0].child.filter((one) => {
        return one[this.keys['id']] === Number(arr[1])
      })
      ...
      const townArr = cityArr[0].child.filter((one) => {
        return one[this.keys['id']] === Number(arr[2])
      })
  ...
  setPos: function(pInd, cInd, tInd) {
    const areaProvince = document.querySelector('.gearArea'+ this.uuid +' .areaProvince')
    const areaCity = document.querySelector('.gearArea'+ this.uuid +' .areaCity')
    const areaCounty = document.querySelector('.gearArea'+ this.uuid +' .areaCounty')
    areaProvince.style.transform = `translate3d(0px, ${-4 * pInd}em, 0px)`
    areaProvince.setAttribute('top', `${-4 * pInd}em)`)
    areaProvince.setAttribute('val', pInd)
    this.setGear(areaProvince, pInd)
    areaCity.setAttribute('val', cInd)
    areaCity.style.transform = `translate3d(0px, ${-4 * cInd}em, 0px)`
    areaCity.setAttribute('top', `${-4 * cInd}em)`)
    this.setGear(areaCity, cInd)
    areaCounty.setAttribute('val', tInd)
    areaCounty.style.transform = `translate3d(0px, ${-4 * tInd}em, 0px)`
    areaCounty.setAttribute('top', `${-4 * tInd}em)`)
  }
}
