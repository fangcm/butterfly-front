    ...
    <div class="content">
      ...
      <p class="row">
        <checker v-model="autoLogin" default-item-class="autologin" selected-item-class="autologin-selected">
          <checker-item :value="'true'"><i
            :class="{'iconfont': true, 'icon-success': autoLogin, 'icon-check-normal': !autoLogin}"></i>自动登录
          </checker-item>
        </checker>
      </p>
...
<script>
  ...
  export default {
    ...
    methods: {
      async login () {
        this.loading = true
        let result = await login({'username': this.userInfo.userName, 'password': this.userInfo.password})
        this.loading = false
